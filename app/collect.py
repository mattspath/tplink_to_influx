...
def do_work(config, influxes):
    ...
    if "kasa" in config:
        for kasa in config["kasa"]["devices"]:
            now_usage_w, now_usage_v, now_usage_a, today_usage, month_usage  = poll_kasa(kasa['ip'])
            ...
            stats[kasa['name']] = {
                    ...
                    "now_usage_v" : now_usage_v,
                    "now_usage_a" : now_usage_a,
                    "time" : start_time
                }
            # stats[kasa['name']] = {
            #         "today_usage" : today_usage,
...
def poll_kasa(ip):
    ...
    # See whether the socket returned monthly readings
    if p.emeter_this_month:
        #Convert from kWh to Wh
        month_usage = p.emeter_this_month * 1000
    ...
    # Convert to volts
    try:
        now_usage_v = usage_dict["voltage"]
        # print(f"Voltage: {now_usage_v}")
    except:
        # An error occurred, let the caller handle it
        print(f'Err: failed to calculate voltage')
        return False, False

    # Convert to amps
    try:
        now_usage_a = usage_dict["current"]
        # print(f"Current: {now_usage_a}")
    except:
        # An error occurred, let the caller handle it
        print(f'Err: failed to calculate amperage')
        return False, False


    return now_usage_w, now_usage_v, now_usage_a, today_usage, month_usage
...
def buildPointsBuffer(points):
    ...
    for point in points:
        ...
        # Build a point    
        p = influxdb_client.Point("power_watts").tag("host", point).field("consumption_w", float(points[point]['now_usage_w'])).time(points[point]['time'])
        points_buffer.append(p)
        
        # If we've captured voltage, add a point for that
        if points[point]['now_usage_v']:
            p = influxdb_client.Point("power_watts").tag("host", point).field("consumption_v", int(float(points[point]['now_usage_v']))).time(points[point]['time'])
            points_buffer.append(p)

        # If we've captured current, add a point for that
        if points[point]['now_usage_a']:
            p = influxdb_client.Point("power_watts").tag("host", point).field("consumption_a", int(float(points[point]['now_usage_a']))).time(points[point]['time'])
            points_buffer.append(p)


        # If we've captured today usage, add a point for that
        ...
        # If we've captured month usage, add a point for that
        if points[point]['month_usage']:
            p = influxdb_client.Point("power_watts").tag("host", point).field("watts_month", int(float(points[point]['month_usage']))).time(points[point]['time'])
            points_buffer.append(p)
